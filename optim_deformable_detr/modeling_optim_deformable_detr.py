...
class OptimDeformableDetrMultiscaleDeformableAttention(nn.Module):
    """
    ...
    def __init__(self, config: DeformableDetrConfig, num_heads: int, n_points: int):
        ...
        if not ((dim_per_head & (dim_per_head - 1) == 0) and dim_per_head != 0):
            warnings.warn(
                "You'd better set embed_dim (d_model) in OptimDeformableDetrMultiscaleDeformableAttention to make the"
...
class OptimDeformableDetrEncoderLayer(nn.Module):
    def __init__(self, config: DeformableDetrConfig):
        super().__init__()
        self.embed_dim = config.d_model
        #! Optim: use OptimDeformableDetrMultiscaleDeformableAttention
        self.self_attn = OptimDeformableDetrMultiscaleDeformableAttention(
            config,
...
class OptimDeformableDetrDecoderLayer(nn.Module):
    def __init__(self, config: DeformableDetrConfig):
        super().__init__()
        self.embed_dim = config.d_model
        ...
        # cross-attention
        #! Optim: use OptimDeformableDetrMultiscaleDeformableAttention
        self.encoder_attn = OptimDeformableDetrMultiscaleDeformableAttention(
            config,
            num_heads=config.decoder_attention_heads,
...
class OptimDeformableDetrPreTrainedModel(PreTrainedModel):
    ...
    _no_split_modules = [
        ...
        r"OptimDeformableDetrEncoderLayer",
        r"OptimDeformableDetrDecoderLayer",
    ]
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, OptimDeformableDetrMultiscaleDeformableAttention):
...
class OptimDeformableDetrEncoder(OptimDeformableDetrPreTrainedModel):
    """
    Transformer encoder consisting of *config.encoder_layers* deformable attention layers. Each layer is a
    [`OptimDeformableDetrEncoderLayer`].
    ...
    def __init__(self, config: DeformableDetrConfig):
        ...
        self.dropout = config.dropout
        #! Optim: use OptimDeformableDetrEncoderLayer
        self.layers = nn.ModuleList(
            [
                OptimDeformableDetrEncoderLayer(config)
    ...
    def forward(
        ...
        #! Optim: pass_in spatial_shapes_list instead of spatial_shapes to avoid tensor data dependent operations
...
class OptimDeformableDetrDecoder(OptimDeformableDetrPreTrainedModel):
    """
    Transformer decoder consisting of *config.decoder_layers* layers. Each layer is a [`OptimDeformableDetrDecoderLayer`].
    ...
    def __init__(self, config: DeformableDetrConfig):
        ...
        self.dropout = config.dropout
        #! Optim: use OptimDeformableDetrDecoderLayer
        self.layers = nn.ModuleList(
            [
                OptimDeformableDetrDecoderLayer(config)
...
class OptimDeformableDetrModel(OptimDeformableDetrPreTrainedModel):
    def __init__(self, config: DeformableDetrConfig):
        super().__init__(config)
        ...
        #! Optim: use OptimDeformableDetrEncoder and OptimDeformableDetrDecoder
        self.encoder = OptimDeformableDetrEncoder(config)
        self.decoder = OptimDeformableDetrDecoder(config)
    ...
    def forward(
        ...
        ```python
        >>> from transformers import AutoImageProcessor, OptimDeformableDetrModel
        ...
        >>> model = OptimDeformableDetrModel.from_pretrained("SenseTime/deformable-detr")
        ...
        #! Optim: create spatial_shapes_list to avoid tensor data dependent operations later on
        spatial_shapes_list = []
        ...
        #! Optim: pass in spatial_shapes_list to avoid tensor data dependent operations later on
...
class OptimDeformableDetrForObjectDetection(OptimDeformableDetrPreTrainedModel):
    ...
    def __init__(self, config: DeformableDetrConfig):
        ...
        self.model = OptimDeformableDetrModel(config)
    ...
    def forward(
        ...
        ```python
        >>> from transformers import AutoImageProcessor, OptimDeformableDetrForObjectDetection
        >>> from PIL import Image
        >>> import requests
        ...
        >>> image_processor = AutoImageProcessor.from_pretrained("SenseTime/deformable-detr")
        >>> model = OptimDeformableDetrForObjectDetection.from_pretrained("SenseTime/deformable-detr")
